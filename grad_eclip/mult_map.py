...
def clip_encode_dense(x, n, clipmodel, clip_inres, clip_ksize):
    ...
    # Extract class embedding properly
    ...
    if hasattr(clipmodel.visual, 'proj') and clipmodel.visual.proj is not None:
...
def main():
    ...
    parser.add_argument("--model", type=str, default="ViT-B-16", help="Name of the model to load from MODELS config")
    ...
    if model_cfg is None:
        print(f"Model '{args.model}' not found in MODELS list. Available models: {[m['name'] for m in MODELS]}")
        return
    ...
    if model_name.startswith("hf-hub:"):
        clipmodel, _, preprocess = open_clip.create_model_and_transforms(model_name, device=device)
    else:
        pretrained = model_cfg.get("pretrained", "")
        clipmodel, _, preprocess = open_clip.create_model_and_transforms(model_name, pretrained=pretrained, device=device)
    
    tokenizer = open_clip.get_tokenizer(model_name)

    # --- Extract Resolution & Kernel Sizes Safely ---
    if hasattr(clipmodel.visual, 'image_size'):
        clip_inres = clipmodel.visual.image_size
        if isinstance(clip_inres, tuple):
            clip_inres = clip_inres[0]
    else:
    ...
    for img_idx, fname in enumerate(jpg_files):
        ...
        # preprocess image and text using open_clip paradigms
        img_preprocessed = preprocess(img).cuda().unsqueeze(0)
        text_processed = tokenizer(texts).cuda()
        ...
        # Call clip_encode_dense passing the required objects natively
        ...
        fig.savefig(f'maps/{args.folder}/{safe_model_name}_heatmap_{img_idx}.png', dpi=600, bbox_inches='tight')
